"""
train_blurpool.py — BlurPool UNet training with all class-collapse fixes applied.

Mirrors all fixes from train_baseline.py:
  ...
  #11 best_blurpool.pth is overwritten by this run
...
def train(args):
    ...
    # Model
    model = get_blurpool_unet(in_channels=4, out_channels=4).to(device)
    avr_dict = {}
    attach_avr_hooks(model, avr_dict)

    # Fix #1 — class weights
    class_weights = torch.tensor([0.1, 1.0, 2.0, 4.0], device=device)
    ...
    for epoch in range(args.max_epochs):
        ...
        if mean_bf1_et > best_bf1_et:
            ...
            torch.save(model.state_dict(), 'results/best_blurpool.pth')
            print(f"  >>> NEW BEST  BF1_ET={best_bf1_et:.4f} -- "
                  f"saved results/best_blurpool.pth <<<")

    # ===================================================================
    # Post-training: save val_metrics_blurpool.csv
    # ===================================================================
    os.makedirs('results', exist_ok=True)

    if final_rows:
        df = pd.DataFrame(final_rows)
        df.to_csv('results/val_metrics_blurpool.csv', index=False)
        print(f"Saved {len(df)} validation rows -> results/val_metrics_blurpool.csv")
        print("\nFirst 5 rows of val_metrics_blurpool.csv:")
        print(df.head().to_string(index=False))
    else:
        print("No validation rows collected - val_metrics_blurpool.csv NOT written.")
    ...
    if epoch_train_losses:
        ...
        ax1.set_title('BlurPool — Training Loss')
        ...
        ax2.set_title('BlurPool — Val BF1_ET per Epoch')
        ax2.legend()
        ax2.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig('results/training_curve_blurpool.png', dpi=150)
        plt.close()
        print("Saved results/training_curve_blurpool.png")
    ...
    if not os.path.exists(target_dir):
        ...
        sanity_model = get_blurpool_unet(in_channels=4, out_channels=4).to(device)
        sanity_model.load_state_dict(
            torch.load('results/best_blurpool.pth', map_location=device))
        sanity_model.eval()
        ...
        with torch.no_grad():
            ...
            print(f"Argmax distribution — {case_id} slice 80 (best_blurpool.pth):")
            for cls in [0, 1, 2, 4]:
